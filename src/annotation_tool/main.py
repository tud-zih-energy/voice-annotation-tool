...
from PySide6.QtWidgets import QApplication, QMainWindow, QFileDialog, QErrorMessage, QMessageBox
from PySide6.QtCore import Slot, QStandardPaths, QTranslator, QLocale, QCommandLineParser
...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def get_settings_file(self):
        """
        Returns the json file which stores a list of recently used projects
        and the shortcuts.
        """
        return os.path.join(self.get_data_dir(), "settings.json")
    ...
    def save_settings(self):
        ...
        """
        with open(self.get_settings_file(), "w") as file:
            json.dump({
...
def main():
    ...
    main = Main()
    main.actionAboutQT.triggered.connect(app.aboutQt)
    sys.excepthook = main.excepthook
    args = parser.positionalArguments()
    if len(args) > 0:
        main.project_opened(Project(args[0]))
    main.show()
    app.exec()
